...
func (fq *FeatureQuery) QueryRole() *RoleQuery {
	query := (&RoleClient{config: fq.config}).Query()
...
func (fq *FeatureQuery) WithRole(opts ...func(*RoleQuery)) *FeatureQuery {
	query := (&RoleClient{config: fq.config}).Query()
...
func (fq *FeatureQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Feature, error) {
	...
	if query := fq.withRole; query != nil {
		if err := fq.loadRole(ctx, query, nodes,
			func(n *Feature) { n.Edges.Role = []*Role{} },
			func(n *Feature, e *Role) { n.Edges.Role = append(n.Edges.Role, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (fq *FeatureQuery) loadRole(ctx context.Context, query *RoleQuery, nodes []*Feature, init func(*Feature), assign func(*Feature, *Role)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(feature.RoleTable)
		s.Join(joinT).On(s.C(role.FieldID), joinT.C(feature.RolePrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(feature.RolePrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(feature.RolePrimaryKey[1]))
