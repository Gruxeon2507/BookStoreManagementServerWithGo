...
func (cq *CategoryQuery) QueryBook() *BookQuery {
...
func (cq *CategoryQuery) WithBook(opts ...func(*BookQuery)) *CategoryQuery {
	query := (&BookClient{config: cq.config}).Query()
...
func (cq *CategoryQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Category, error) {
	...
	if query := cq.withBook; query != nil {
		if err := cq.loadBook(ctx, query, nodes,
			func(n *Category) { n.Edges.Book = []*Book{} },
			func(n *Category, e *Book) { n.Edges.Book = append(n.Edges.Book, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (cq *CategoryQuery) loadBook(ctx context.Context, query *BookQuery, nodes []*Category, init func(*Category), assign func(*Category, *Book)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(category.BookTable)
		s.Join(joinT).On(s.C(book.FieldID), joinT.C(category.BookPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(category.BookPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(category.BookPrimaryKey[1]))
