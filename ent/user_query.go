...
func (uq *UserQuery) QueryCreatedBy() *BookQuery {
...
func (uq *UserQuery) WithCreatedBy(opts ...func(*BookQuery)) *UserQuery {
	query := (&BookClient{config: uq.config}).Query()
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := uq.withCreatedBy; query != nil {
		if err := uq.loadCreatedBy(ctx, query, nodes,
			func(n *User) { n.Edges.CreatedBy = []*Book{} },
			func(n *User, e *Book) { n.Edges.CreatedBy = append(n.Edges.CreatedBy, e) }); err != nil {
			return nil, err
		}
	}
...
func (uq *UserQuery) loadCreatedBy(ctx context.Context, query *BookQuery, nodes []*User, init func(*User), assign func(*User, *Book)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(book.FieldCreatedBy)
	}
	...
	for _, n := range neighbors {
		...
		node, ok := nodeids[fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "createdBy" returned %v for node %v`, fk, n.ID)
		}
		assign(node, n)
	}
	return nil
