...
func (rq *RoleQuery) QueryFeature() *FeatureQuery {
	query := (&FeatureClient{config: rq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := rq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (rq *RoleQuery) Clone() *RoleQuery {
	...
	return &RoleQuery{
		...
		withFeature: rq.withFeature.Clone(),
...
func (rq *RoleQuery) WithFeature(opts ...func(*FeatureQuery)) *RoleQuery {
	query := (&FeatureClient{config: rq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	rq.withFeature = query
	return rq
}
...
func (rq *RoleQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Role, error) {
	...
	if query := rq.withFeature; query != nil {
		if err := rq.loadFeature(ctx, query, nodes,
			func(n *Role) { n.Edges.Feature = []*Feature{} },
			func(n *Role, e *Feature) { n.Edges.Feature = append(n.Edges.Feature, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (rq *RoleQuery) loadUser(ctx context.Context, query *UserQuery, nodes []*Role, init func(*Role), assign func(*Role, *User)) error {
	...
	query.Where(func(s *sql.Selector) {
		...
		s.Join(joinT).On(s.C(user.FieldID), joinT.C(role.UserPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(role.UserPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(role.UserPrimaryKey[0]))
...
func (rq *RoleQuery) loadFeature(ctx context.Context, query *FeatureQuery, nodes []*Role, init func(*Role), assign func(*Role, *Feature)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[int]*Role)
	nids := make(map[int]map[*Role]struct{})
	for i, node := range nodes {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(role.FeatureTable)
		s.Join(joinT).On(s.C(feature.FieldID), joinT.C(role.FeaturePrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(role.FeaturePrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(role.FeaturePrimaryKey[0]))
		s.AppendSelect(columns...)
		s.SetDistinct(false)
	})
	if err := query.prepareQuery(ctx); err != nil {
